...
void SubdividePolygon(int32_t numverts, float* verts)
{
	...
	poly = Memory_HunkAlloc(sizeof(glpoly_t) + ((numverts - 4) + 2) * VERTEXSIZE * sizeof(float));
...
void EmitWaterPolys(msurface_t* fa)
{
	...
	if (fa->texinfo->flags & SURF_FLOWING)
		scroll = -64.0f * ((r_newrefdef.time * 0.5f) - (int32_t)(r_newrefdef.time * 0.5f));
	else
		scroll = 0;
...
void R_SetSky(char* name, float rotate, vec3_t axis)
{
	...
	for (i = 0; i < 6; i++)
	{
		...
		snprintf(pathname, sizeof(pathname), "env/%s%s.tga", skyname, suf[i]);
