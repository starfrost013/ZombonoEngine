...
/*
=============
Draw_GetScaledAssetPath

Obtains scaled asset path
=============
*/
...
char* Draw_GetScaledImagePath(char* name)
{
	...
	{
		snprintf(scaled_asset_path, MAX_QPATH, "%s@0.25x", name);
	}
...
=============
*/
void Draw_Pic(int32_t x, int32_t y, char* pic, color4_t color, bool use_scaled_assets)
{
	image_t* gl;
	cvar_t* scale = ri.Cvar_Get("hudscale", "1", 0);

	if (use_scaled_assets)
		pic = Draw_GetScaledImagePath(pic);
...
================
*/
void Draw_PicArea(int32_t x, int32_t y, int32_t start_x, int32_t start_y, int32_t end_x, int32_t end_y, char* pic, color4_t color, float scale, bool use_scaled_assets)
{
	image_t* gl;
...
=============
*/
void Draw_PicStretch(int32_t x, int32_t y, int32_t w, int32_t h, char* pic, color4_t color, bool use_scaled_assets)
{
	image_t* gl;
...
{
	cvar_t* scale = ri.Cvar_Get("hudscale", "1", 0);
	Draw_PicArea(x, y, start_x, start_y, end_x, end_y, pic, color, scale->value, use_scaled_assets);
}
...
void Draw_FontChar(int32_t x, int32_t y, int32_t start_x, int32_t start_y, int32_t end_x, int32_t end_y, char* pic, color4_t color, bool use_scaled_assets)
{
	...
	Draw_PicArea(x, y, start_x, start_y, end_x, end_y, pic, color, font_scale, false);
}
