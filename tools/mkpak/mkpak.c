/* mkpak.c 
Make Zombono engine (Euphoria) PAK files
...
static size_t recurse_directory(pathbuf* pb, int32_t w) {
    size_t count = 0;
...
static size_t enter_directory(char* path, int32_t should_write) {
    pathbuf pb;
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 3) {
        fprintf(stderr, "usage: %s [input directory] [output archive]\n"\
